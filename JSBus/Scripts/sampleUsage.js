(function () {
    ...
    var testHub = $.connection.test,
        ...
        bus = new JSBus.Bus("test", sendTransport, subscribeTransport),
        
        // Below variables are needed only for visualizing traffic, not normally needed
    ...
    // subscribe to send transport notifications to show status on UI - this is not normally needed
    sendTransport.notifyWhenSent(function (m) {
        status.outgoingQueue.value--;
        status.sent.value++;
    });

    // Subscribe to transport ack to show demonstration on UI - this is not normally needed
    ...
    // Subscibe to business event to visualize when answer is returned from server
